...
export default function ProductDetailsView() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      // استخراج ID من الرابط بطريقة آمنة
  ...
  if (loading) {
    return (
      <div className="max-w-md mx-auto min-h-screen bg-white p-6 space-y-4">
        ...
        {related.length > 0 && (
          <div className="pt-10 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {related.map((item) => (
                <div key={item.id} onClick={() => window.location.href = `/product/${item.id}`} className="cursor-pointer group">
